...
public class DefaultOpenAiSession implements OpenAiSession {
    ...
    /**
     * openAiApi.completions(qaCompletionRequest).blockingGet()
     * 下面的completions方法都是因为openAiApi由那个Retroit类库生成 所以可以直接用
     */
    @Override
    ...
    public EventSource completions(QACompletionRequest qaCompletionRequest, EventSourceListener eventSourceListener) throws JsonProcessingException {
        // 核心参数校验；qaCompletionRequest必须为stream   如果用户不是传的流式传输不对用户的传参做更改，只返回错误信息。
        ...
        Request request = new Request.Builder()
                .url(configuration.getApiHost().concat(IOpenAiApi.v1_completions))//拼接发给apihost的请求
    ...
    public EventSource chatCompletions(ChatCompletionRequest chatCompletionRequest, EventSourceListener eventSourceListener) throws JsonProcessingException {
        return this.chatCompletions(Constants.NULL,Constants.NULL,chatCompletionRequest,eventSourceListener);
    }
        @Override
